...
@section('content')
    ...
    <div class="bg-medium-gray rounded-xl shadow-lg p-6 animate-fadeIn max-w-4xl mx-auto">
        <h2 class="text-xl font-bold mb-6">Editar Seção Personalizada</h2>
        <x-alert />

        <form action="{{ route('admin.sections.update', $section->id) }}" method="POST" id="sectionForm">
            @csrf
            @method('PUT')
                ...
                <ul id="selectedItems" class="space-y-2">
                    @foreach ($section->items as $i => $item)
                        <li class="bg-gray-700 p-2 rounded flex items-center justify-between gap-4">
                            <div>
                                <strong>{{ $item->content->title ?? ($item->content->name ?? 'Sem título') }}</strong>
                                <small class="text-sm">({{ $item->content_type }})</small>
                                <input type="hidden" name="items[{{ $i }}][content_id]"
                                    value="{{ $item->content_id }}">
                                <input type="hidden" name="items[{{ $i }}][content_type]"
                                    value="{{ $item->content_type }}">
                            </div>
                            <div class="flex gap-2 items-center">
                                <label>Ordem:</label>
                                <input type="number" name="items[{{ $i }}][order]" value="{{ $item->order }}"
                                    class="w-20 bg-dark-gray border border-gray-700 rounded-lg py-1 px-2 text-white">
                                <button type="button"
                                    class="removeItem bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded">
                                    Remover
                                </button>
                            </div>
                        </li>
