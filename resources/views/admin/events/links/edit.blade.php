...
<div class="bg-medium-gray rounded-xl shadow-lg p-6 animate-fadeIn">
    <h2 class="text-xl font-bold mb-6">
        Editar Transmissão — {{ $event->title }}
    </h2>

    <x-alert />

    <form action="{{ route('admin.events.links.update', [$event->id, $link->id]) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="type"
                        class="w-full bg-dark-gray border border-gray-700 rounded-lg py-2 px-4 text-white">
                    @foreach(['m3u8','mp4','embed','custom'] as $type)
                        <option value="{{ $type }}" @selected($link->type === $type)>
                            {{ strtoupper($type) }}
                        </option>
                    @endforeach
                </select>
